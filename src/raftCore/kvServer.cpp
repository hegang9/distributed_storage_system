...
void KvServer::Get(const raftKVRpcProctoc::GetArgs *args, raftKVRpcProctoc::GetReply *reply) {
  ...
  int _ = -1;
  bool isLeader = false;
  m_raftNode->Start(op, &raftIndex, &_, &isLeader);  // raftIndex 为 Raft 预计的日志索引
  ...
  if (!chForRaftIndex->timeOutPop(CONSENSUS_TIMEOUT, &raftCommitOp)) {
    ...
    if (ifRequestDuplicate(op.ClientId, op.RequestId) && isLeader) {
      ...
      reply->set_err(ErrWrongLeader);  // 让客户端切换 Leader 重试
    }
  } else {
    // 已提交：继续执行并返回结果
...
// 判断请求是否重复（根据 clientId 和 requestId）
...
// Put/Append 的执行逻辑：需要通过 Raft 达成一致后再返回
...
// 构造函数：启动 RPC 服务，连接 Raft 节点并启动 apply 循环
